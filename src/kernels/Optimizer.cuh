...
void adamWOptimizerStep(
        ...
        const float epsilon,
        const float batchSize, // This parameter is now unused but we keep it for API consistency
        const uint size) {
    const uint tIdx = blockIdx.x * blockDim.x + threadIdx.x;
    if (tIdx >= size) return;

    const float gradValue = batchGradTensor[tIdx];

    
    // --- The rest of the kernel is correct ---
    
    const float mMomentUpdated = beta1 * mMomentTensor[tIdx] + (1.0f - beta1) * gradValue;
    mMomentTensor[tIdx] = mMomentUpdated;
    
    const float gradValueSquared = gradValue * gradValue;
    const float vMomentUpdated = beta2 * vMomentTensor[tIdx] + (1.0f - beta2) * gradValueSquared;
    vMomentTensor[tIdx] = vMomentUpdated;
    
    const float mMomentHat = mMomentUpdated / bias_correction1;
    const float vMomentHat = vMomentUpdated / bias_correction2;
